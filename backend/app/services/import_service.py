...
_ROM_EXTENSIONS = {
    "nes", "smc", "sfc", "smd", "bin", "gb", "gbc", "gba", "nds", "3ds",
    "cia", "iso", "cso", "pbp", "pkg", "vpk", "nsp", "xci", "nsz", "xcz",
    "wbfs", "wad", "gcm", "rvz", "wux", "wud", "cdi", "gdi", "cue", "chd",
    "img", "rom", "v64", "z64", "n64", "zip",
}
...
async def import_downloaded_file(db: Session, item: QueueItem) -> dict:
    ...
    if not raw_path:
        raise ValueError("Download client did not return a file path — download may still be in progress or was removed")
    ...
    if not rom_files:
        raise ValueError(
            ...
            + (f" (expected extensions: {', '.join(sorted(platform_exts))})" if platform_exts else "")
        )
