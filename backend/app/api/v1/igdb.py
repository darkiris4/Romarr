...
def get_collection_games(collection_id: int):
    """Return all games in an IGDB collection/series."""
    client_id, _ = _credentials()
    token = _get_token()
    if not token:
        return []
    ...
    body = (
        f"fields id, name, first_release_date, cover.image_id, summary,"
        f" platforms.id, platforms.name, total_rating, aggregated_rating, rating,"
        f" release_dates.region;"
        f" where collections = ({collection_id}); limit 50;"
    )
    results = _igdb_query(client_id, token, body)
    results.sort(key=lambda g: g.get("first_release_date") or float("inf"))
