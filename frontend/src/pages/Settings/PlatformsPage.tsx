        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            <div className="form-group">
              ...
              <input className="form-control" value={name} onChange={e => setName(e.target.value)} required placeholder="Super Nintendo Entertainment System" />
            </div>
            <div className="form-group">
              <label className="form-label">No-Intro DAT Name</label>
              <input className="form-control" value={noIntroName} onChange={e => setNoIntroName(e.target.value)} required placeholder="Nintendo - Super Nintendo Entertainment System" />
            ...
            <div className="form-group">
              ...
              <input className="form-control" value={folderName} onChange={e => setFolderName(e.target.value)} required placeholder="Nintendo - Super Nintendo Entertainment System" />
            </div>
            <div className="form-group">
              <label className="form-label">File Extensions</label>
              <input className="form-control" value={extensions} onChange={e => setExtensions(e.target.value)} placeholder="sfc,smc" />
...
export default function PlatformsPage() {
  ...
  const { data: platforms = [], isLoading } = useQuery({ queryKey: ['platforms'], queryFn: platformsApi.list })
  const { data: builtins = [] } = useQuery({ queryKey: ['platforms-builtin'], queryFn: platformsApi.builtin })

  const createMutation = useMutation({
    mutationFn: platformsApi.create,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['platforms'] }); setShowForm(false) },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<Platform> }) => platformsApi.update(id, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['platforms'] }); setEditTarget(null) },
  })
  ...
  return (
    <div>
      ...
      <div className="settings-section-desc">Configure which platforms Romarr manages. Platform folder names must match No-Intro conventions.</div>
                  ...
                  <tr key={p.id}>
                    ...
                    <td>
                      <label className="toggle">
                        <input
                          type="checkbox"
                          checked={p.enabled}
                          onChange={e => updateMutation.mutate({ id: p.id, data: { enabled: e.target.checked } })}
                        />
                    ...
                    <td>
                      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                        <button className="btn-icon" onClick={() => deleteMutation.mutate(p.id)}><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <div className="card">
        <div className="card-header"><span className="card-title">Common Platforms</span></div>
