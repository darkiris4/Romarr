...
export default function CalendarPage() {
  const { data: games = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      client.get<Game[]>('/calendar', { params: { start: YEAR_START, end: YEAR_END } }).then(r => r.data),
  })
        ...
        .map(([year, yearGames]) => (
          <div key={year} className="settings-section">
            <div className="card-header">
              ...
              <span className="text-muted text-sm">{yearGames.length} game{yearGames.length !== 1 ? 's' : ''}</span>
            </div>
